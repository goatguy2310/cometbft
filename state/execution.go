...
func (blockExec *BlockExecutor) ProcessProposal(
	...
	resp, err := blockExec.proxyApp.ProcessProposal(context.TODO(), &abci.ProcessProposalRequest{
		...
		ProposedLastCommit: buildLastCommitInfoFromStore(block, blockExec.store, state.InitialHeight, blockExec.NStates),
		Misbehavior:        block.Evidence.Evidence.ToABCI(),
		ProposerAddress:    block.ProposerAddress,
		NextValidatorsHash: block.NextValidatorsHash,
	})
...
func (blockExec *BlockExecutor) ApplyBlock(
	...
	if !blockExec.lastValidatedBlock.HashesTo(block.Hash()) {
		if err := validateBlock(state, block, blockExec.NStates); err != nil {
...
func (blockExec *BlockExecutor) applyBlock(state State, blockID types.BlockID, block *types.Block, syncingToHeight int64) (State, error) {
	...
	abciResponse, err := blockExec.proxyApp.FinalizeBlock(context.TODO(), &abci.FinalizeBlockRequest{
		...
		Height:             block.Height,
		Time:               block.Time,
		DecidedLastCommit:  buildLastCommitInfoFromStore(block, blockExec.store, state.InitialHeight, blockExec.NStates),
		Misbehavior:        block.Evidence.Evidence.ToABCI(),
		Txs:                block.Txs.ToSliceOfBytes(),
...
func buildLastCommitInfoFromStore(block *types.Block, store Store, initialHeight int64, NStates int) abci.CommitInfo {
	if block.Height < initialHeight+int64(NStates) { // check for initial height before loading validators
		// there is no last commit for the initial height.
		// return an empty value.
		return abci.CommitInfo{}
	}

	lastValSet, err := store.LoadValidators(block.Height - int64(NStates))
	if err != nil {
...
func ExecCommitBlock(
	...
	commitInfo := buildLastCommitInfoFromStore(block, store, initialHeight, NStates)
